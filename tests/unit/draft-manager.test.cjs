...
test('ripristina e scarta soltanto la revisione letta', async () => {
  ...
  assert.equal(await session.discard(), true);
...
test('chiude la sessione senza rejection e segnala un autosalvataggio fallito', async () => {
  const warnings = [];
  const context = loadAppScripts(['js/drafts/draft-manager.js'], {
    console: {
      warn(...args) {
  ...
  const repeatedOutcome = await session.close();

  assert.equal(outcome.closed, true);
  assert.match(outcome.error.message, /Quota esaurita/);
  assert.equal(repeatedOutcome, outcome);
